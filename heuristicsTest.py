...
for i in range(len(initial_route) - 1):
    if distance_matrix.at[initial_route[i], initial_route[i + 1]] != np.inf:
        connected_nodes.append(initial_route[i])
connected_nodes.append(initial_route[-1])

initial_route = connected_nodes
initial_distance = calculate_total_distance(initial_route)

print(f"Updated Initial Route: {initial_route}")
print(f"Updated Initial Distance: {initial_distance/1000:.2f} km")
