...
# Ant Colony Optimization (ACO) Algorithm with Dynamic Decay and Simulated Annealing
class AntColony:
    def __init__(self, distance_matrix, num_ants, num_iterations, initial_decay=0.85, alpha=1.5, beta=2.5):
    ...
    def run(self):
        for iteration in range(self.num_iterations):
            ...
            decay = self.initial_decay + (0.05 * (iteration / self.num_iterations))
            all_routes = self.generate_all_routes()
            self.spread_pheromone(all_routes, decay)
    ...
    def spread_pheromone(self, all_routes, decay):
    ...
    def refine_with_simulated_annealing(self, route):
        current_route = route
...
print("Starting ACO optimization with dynamic decay and SA refinement...")
ant_colony = AntColony(distance_matrix.values, num_ants, num_iterations, initial_decay, alpha, beta)
ani = FuncAnimation(fig, update, frames=ant_colony.run(), blit=True, repeat=False, interval=100, cache_frame_data=False)
plt.show()

# After animation is complete, refine with Simulated Annealing if needed
optimized_route = ant_colony.final_best_route
optimized_distance = ant_colony.final_best_distance
optimized_route, optimized_distance = ant_colony.refine_with_simulated_annealing(optimized_route)
